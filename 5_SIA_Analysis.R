...
GULD_SIA1 <- read.csv("~/trophicpersonalities_GULD/4_SIA_Analysis/SIA_batch1.csv")
labels(GULD_SIA1)
...
hist(GULD_SIA1$d15N)
hist(GULD_SIA1$d13C)
hist(GULD_SIA1$N_percent)
hist(GULD_SIA1$C_percent)
GULD_SIA1$CN_ratio <- (GULD_SIA1$C_percent/GULD_SIA1$N_percent) #C:N ratios above 4 require lipid correction (Post et al. 2007). All values here below 4.
...
#Variance estimates
GULD_SIA1.Nmod <- lmer(d15N ~ (1|fishID), data=GULD_SIA1)
GULD_SIA1.Nmod
confint(GULD_SIA1.Nmod)
plot(GULD_SIA1.Nmod) 
rpt(d15N ~ (1 | fishID), grname = "fishID", data = GULD_SIA1, datatype = "Gaussian", 
    nboot = 100, npermut = 0)

GULD_SIA1.Cmod <- lmer(d13C ~ (1|fishID), data=GULD_SIA1)
GULD_SIA1.Cmod
confint(GULD_SIA1.Cmod)
plot(GULD_SIA1.Cmod) 
...
GULD_SIA1.plot <- setDT(GULD_SIA1)[ , list(d15N_mean = mean(d15N), d15N_sd = sd(d15N),
                   ...
                   by = .(fishID)]
range(GULD_SIA1.plot$d15N_mean) #range 9.7 - 13.5
range(GULD_SIA1.plot$d13C_mean) #range -22.1 - -13.4
